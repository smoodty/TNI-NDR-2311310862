...
# === Summary Page ===
elif menu == "📊 สรุปข้อมูล":
    st.markdown("## 🧾 สรุปข้อมูลหุ้น Tesla (TSLA80)")
    st.markdown("#### ข้อมูลย้อนหลัง 6 เดือน")
    st.dataframe(df_6mo.reset_index(drop=True))

    st.markdown("### 📌 สถิติเบื้องต้น")
    st.write(df_6mo["ราคาปิด"].describe())

    st.markdown("### 🥇 วันที่ราคาปิดสูงสุด")
    st.dataframe(df_6mo[df_6mo["ราคาปิด"] == df_6mo["ราคาปิด"].max()])

    st.markdown("### 📈 ความสัมพันธ์กับ SET Index")
    if "SET Index" in df_6mo.columns:
